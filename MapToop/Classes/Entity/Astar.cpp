...
void AStar::SetMapData(std::map<int, std::map<int, std::vector<int>>> mapData)
{
	...
	for (auto & Map : mapData)
	{
		for (auto & data : Map.second)
		{
			...
			node->m_posX = Map.first;
			node->m_posY = data.first;
			node->SetNodeType(data.second);
			if (m_mapNodes.find(Map.first) == m_mapNodes.end())
			{
				std::map<int, ASNode *> temp;
				temp.clear();
				temp.insert(std::make_pair(data.first, node));
				m_mapNodes.insert(std::make_pair(Map.first, temp));
			}
			else
			{
				m_mapNodes.at(Map.first).insert(std::make_pair(data.first, node));
			}
		}
	}
}
bool AStar::FindPath(int posx, int posy, int s_posx, int s_posy, std::vector<int> canNotMoveTypes)
{
	...
	do
	{
		...
		if (minNode == findNode){ return false; }
		findNode = minNode;
